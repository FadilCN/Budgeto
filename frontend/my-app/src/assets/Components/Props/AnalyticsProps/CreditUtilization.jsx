...
function CreditUtilization({ used = 0, limit = 1, preUsed = 0, preLimit = 1 }) {
    const ratio = limit ? (used / limit) * 100 : 0;
    const preRatio = preLimit ? (preUsed / preLimit) * 100 : 0;

    const diff = Math.abs(ratio - preRatio);
    const key = ratio > preRatio ? 1 : 0; // higher utilization is worse
